...
func initplayer(first_name string, last_name string, id int) Player {
	player := Player{first_name, last_name, id ,ALLROOM[3001]}
	return player
}
...
func createplayer() error{
	...
	fmt.Printf("Type in your last name?\n")
	fmt.Scanln(&last)

	id := len(ALLPLAYER)
	player := initplayer(first, last, id)
...
func main () {
	...
	if err != nil {
		fmt.Printf("Starting Transaction: %v", err)
	}
	...
	err = createplayer() 
	if err != nil {
		fmt.Printf("Creating Player: %v", err)
	}
