...
func createDatabase(path string) (*sql.DB, error) {
	...
	if err != nil {
		log.Fatalf("create opentransaction:%v\n", err)
		return nil, err
	}
	_, err = tx.Exec("create table pairs (key text, value text)")
	if err != nil {
		log.Fatalf("db.Exec: %v\n", err)
		return nil, err
	}
	if err = tx.Commit(); err != nil {
		log.Fatalf("create tx.Commit: %v\n", err)
		return nil, err
	}
	defer db.Close()
	return db, nil
...
func splitDatabase(source, outputDir, outputPattern string, m int) ([]string, error) {
	...
	if err != nil {
		log.Fatalf("open transaction: %v\n", err)
		return nil, err
	}

	rows, err := transaction.Query("select key, value from pairs")
	if err != nil {
		log.Fatalf("transaction.Query: %v\n", err)
		return nil, err
	}
	defer rows.Close()
