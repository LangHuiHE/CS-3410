...
func (task *ReduceTask) Process(tempdir string, client Interface) error {
	inputDB, err := mergeDatabases(task.SourceHosts, filepath.Join(tempdir, reduceInputFile(task.N)), tempdir)
	if err != nil {
		return err
	}
			...
			if previous != "" {
				...
				go func() {
					for i := range output {
						...
						if err != nil {
							log.Fatalf("ReduceTask.Process db.Exec: %v\n", err)
						}
					}
				}()
				input = make(chan string)
...
func startNewReduce() {
	output := make(chan Pair)
	value_chan := make(chan string)
	return value_chan, output
}

func endOldReduce(outputDB *sql.DB, output chan Pair, value_chan chan string) error {
	var key, value string
	close(value_chan)
	for {
		if output == nil {
	...
	_, err := outputDB.Exec(`insert into pairs (key, value) values (?, ?)`, key, value)
	if err != nil {
		log.Fatalf("ReduceTask.Process db.Exec: %v\n", err)
		return err
	}
