...
type Proposer struct {
	restart       int
	number        int
	preparevote   map[int]*Vote
	acceptvote    map[int]*Vote
	acceptrequest map[int]*Vote
}
...
func main() {
	...
	proposer := make(map[int]*Proposer)
	state := State{network: network, proposer: proposer}
	for scanner.Scan() {
		line := scanner.Text()
